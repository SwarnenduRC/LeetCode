...
void SudokuSolver::initializeBoard(const std::array<int, 81>& board) noexcept
{
    ...
    {
        size_t colIdx = 0;
        std::array<int, ARRAY_LENGTH> row;  // Take an array of 9 elements which will act as row
    ...
    populateValues();   // Populates the respective bitset arrays now with the values
}
